...
VulkanTextOverlay::~VulkanTextOverlay()
{
	...
	vkDestroyPipelineCache(vulkanDevice->mLogicalDevice, mPipelineCache, nullptr);
	vkDestroyPipeline(vulkanDevice->mLogicalDevice, mPipeline, nullptr);
...
void VulkanTextOverlay::prepareResources()
{
	...
	VK_CHECK_RESULT(vkAllocateCommandBuffers(vulkanDevice->mLogicalDevice, &cmdBufAllocateInfo, mCmdBuffers.data()));
	...
	// Font texture
	VkImageCreateInfo imageInfo = InitImageCreateInfo();
	...
	VkMemoryAllocateInfo allocInfo = InitMemoryAllocateInfo();
	...
	VkImageViewCreateInfo imageViewInfo = InitImageViewCreateInfo();
	...
	poolSizes[0] = InitDescriptorPoolSize(
	...
	setLayoutBindings[0] = InitDescriptorSetLayoutBinding(
	...
	writeDescriptorSets[0] = InitWriteDescriptorSet(descriptorSet,
	...
	VK_CHECK_RESULT(vkCreatePipelineCache(vulkanDevice->mLogicalDevice, &pipelineCacheCreateInfo, nullptr, &mPipelineCache));
...
void VulkanTextOverlay::preparePipeline()
{
	...
	VkPipelineColorBlendAttachmentState blendAttachmentState =
			InitPipelineColorBlendAttachmentState(0xf, VK_TRUE);
	...
	vertexBindings[0] = InitVertexInputBindingDescription(0,
														  sizeof(glm::vec4),
														  VK_VERTEX_INPUT_RATE_VERTEX);
	vertexBindings[1] = InitVertexInputBindingDescription(1,
														  sizeof(glm::vec4),
														  VK_VERTEX_INPUT_RATE_VERTEX);

	std::array<VkVertexInputAttributeDescription, 2> vertexAttribs = {};
	// Position
	vertexAttribs[0] = InitVertexInputAttributeDescription(0, 0,
														   VK_FORMAT_R32G32_SFLOAT,
														   0);
	// UV
	vertexAttribs[1] = InitVertexInputAttributeDescription(1, 1,
														   VK_FORMAT_R32G32_SFLOAT,
														   sizeof(glm::vec2));

	VkPipelineVertexInputStateCreateInfo inputState = InitPipelineVertexInputStateCreateInfo();
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->mLogicalDevice, mPipelineCache, 1, &pipelineCreateInfo, nullptr, &mPipeline));
}
...
void VulkanTextOverlay::prepareRenderPass()
{
	...
	VK_CHECK_RESULT(vkCreateRenderPass(vulkanDevice->mLogicalDevice, &renderPassInfo, nullptr, &mRenderPass));
}
...
void VulkanTextOverlay::updateCommandBuffers()
{
	...
	for (int32_t i = 0; i < mCmdBuffers.size(); ++i)
	{
		...
		{
			VksDebugMarkerBeginRegion(mCmdBuffers[i], "Text overlay", glm::vec4(1.0f, 0.94f, 0.3f, 1.0f));
		}

		vkCmdBeginRenderPass(mCmdBuffers[i], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

		VkViewport viewport = InitViewport((float) *frameBufferWidth,
		...
		vkCmdBindVertexBuffers(mCmdBuffers[i], 0, 1, &vertexBuffer.buffer, &offsets);
		vkCmdBindVertexBuffers(mCmdBuffers[i], 1, 1, &vertexBuffer.buffer, &offsets);
		...
		if (gVksDebugMarkerActive)
		{
			VksDebugMarkerEndRegion(mCmdBuffers[i]);
		}
...
*/
void VulkanTextOverlay::submit(VkQueue queue, uint32_t bufferindex, VkSubmitInfo submitInfo)
{
	if (!visible)
	{
		return;
