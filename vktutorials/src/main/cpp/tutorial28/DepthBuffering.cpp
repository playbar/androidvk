...
const char* MODEL_PATH =   "tutorial28/chalet.obj";
const char* TEXTURE_PATH = "tutorial28/texture.jpg";
...
void DepthBuffering::initVulkan(android_app* app)
{
    if( initialized_ )
        return;
    mAndroidAppCtx = app;
...
void DepthBuffering::deleteVulkan()
{
...
bool DepthBuffering::isVulkanReady()
{
    return initialized_;
}

void DepthBuffering::updateUniformBuffer()
{
...
void DepthBuffering::setupDebugCallback()
{
    if( !enableValidationLayers)
        return;
...
bool DepthBuffering::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
bool DepthBuffering::isDeviceSuitable_1(VkPhysicalDevice device) {
...
bool DepthBuffering::isDeviceSuitable(VkPhysicalDevice device)
{
...
QueueFamilyIndices DepthBuffering::findQueueFamilies(VkPhysicalDevice device)
{
...
SwapChainSupportDetails DepthBuffering::querySwapChainSupport(VkPhysicalDevice device)
{
...
VkSurfaceFormatKHR DepthBuffering::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats)
{
...
VkPresentModeKHR DepthBuffering::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>availablePresentModes)
{
...
VkExtent2D DepthBuffering::chooseSwapExtent(const VkSurfaceCapabilitiesKHR & capabilities)
{
...
std::vector<char> DepthBuffering::readFile(const char* filePath)
{
...
void DepthBuffering::createShaderModule(const std::vector<char>& code, VDeleter<VkShaderModule>& shaderModule)
{
...
void DepthBuffering::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
void DepthBuffering::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
{
    auto vertShaderCode = readFile("shaders/shader_28.vert.spv");
    auto fragShaderCode = readFile("shaders/shader_28.frag.spv");
...
VkFormat DepthBuffering::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
...
bool DepthBuffering::hasStencilComponent(VkFormat format)
{
...
void DepthBuffering::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VDeleter<VkImage>& image, VDeleter<VkDeviceMemory>& imageMemory)
{
...
void DepthBuffering::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
void DepthBuffering::copyImage(VkImage srcImage, VkImage dstImage, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkCommandBuffer DepthBuffering::beginSingleTimeCommands() {
    VkCommandBufferAllocateInfo allocInfo = {};
...
void DepthBuffering::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
    vkEndCommandBuffer(commandBuffer);
...
void DepthBuffering::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VDeleter<VkImageView> &imageView)
{
...
void DepthBuffering::createUniformBuffer()
{
...
void DepthBuffering::createDescriptorSet()
{
...
void DepthBuffering::createVertexBuffer_1()
{
    VkBufferCreateInfo bufferInfo = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
...
uint32_t DepthBuffering::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void DepthBuffering::createSemaphores()
{
